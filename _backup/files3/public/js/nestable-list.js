...
class NestableList {
    ...
    createItemElement(item) {
        ...
        li.addEventListener('dragover', (e) => {
            ...
            const closestItem = target?.closest('.nestable-item');
    ...
    getDropZone(element, clientY) {
        ...
        const handleBottom = handleRect.bottom - rect.top;
        const handleHeight = handleRect.height;

        // Zona superior: acima do handle (top 25% do handle ou área acima)
        const topZoneThreshold = handleTop + (handleHeight * 0.25);

        // Zona inferior: abaixo do handle (bottom 25% do handle ou área abaixo)
        const bottomZoneThreshold = handleBottom - (handleHeight * 0.25);

        // Verificar se está sobre o handle (zona do meio)
        if (relativeY >= handleTop && relativeY <= handleBottom) {
            // Está sobre o handle = tornar filho
            return 'drop-child';
        } else if (relativeY < topZoneThreshold) {
            // Zona superior = irmão acima
            return 'drop-above';
        } else if (relativeY > bottomZoneThreshold) {
            // Zona inferior = irmão abaixo
            return 'drop-below';
        }

        // Se está entre as zonas, considerar como handle (tornar filho)
        return 'drop-child';
    }
    ...
    saveEditItem() {
        ...
        const itemElement = this.list.querySelector(`[data-id="${this.currentEditItem}"]`);
        if (itemElement) {
            const nameElement = itemElement.querySelector('.item-name');
            if (nameElement) {
