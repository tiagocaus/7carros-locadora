...
@section('title', t('modules.mensageria.offcanvas.edit_sms'))
...
<div class="p-4">
    ...
    <form id="formEditarConexaoSMS" class="hidden">
        <div class="mb-4">
            <label class="block text-sm font-medium text-slate-700 mb-1"><?= t('modules.mensageria.sms.sender_id') ?> *</label>
            <input type="text" name="sender_id" id="inputSenderId" class="form-input-focus w-full" maxlength="11">
        </div>
        <div class="mb-4">
            <label class="block text-sm font-medium text-slate-700 mb-1"><?= t('modules.mensageria.sms.username') ?></label>
            <input type="text" name="username" id="inputUsername" class="form-input-focus w-full" placeholder="<?= t('modules.mensageria.smtp.keep_blank') ?>">
        </div>
        <div class="mb-4">
            <label class="block text-sm font-medium text-slate-700 mb-1"><?= t('modules.mensageria.sms.api_key') ?></label>
            <input type="password" name="api_key" class="form-input-focus w-full" placeholder="<?= t('modules.mensageria.smtp.keep_blank') ?>">
            <p class="text-xs text-slate-500 mt-1"><?= t('modules.mensageria.sms.api_key_change_hint') ?></p>
        </div>
        <div class="mb-4">
...
(function() {
    const i18n = {
        ...
        smsUpdateError: <?= js_t("modules.mensageria.messages.sms_update_error") ?>,
    ...
    async function carregarDados() {
        try {
            ...
            // Preencher campos
            document.getElementById('inputSenderId').value = conexao.sender_id || '';
            ...
            document.getElementById('formEditarConexaoSMS').classList.remove('hidden');
    ...
    // Submit
    document.getElementById('formEditarConexaoSMS')?.addEventListener('submit', async function(e) {
        e.preventDefault();
        ...
        try {
            const result = await API.post(`/sms/${conexaoId}/atualizar`, dados);
            ...
            mostrarToast(i18n.smsUpdateError, 'error');
            btnSubmit.disabled = false;
            btnSubmit.innerHTML = `<i class="fas fa-save mr-2"></i>${i18n.saveChanges}`;
        }
    });
