...
@section('title', t('modules.relatorios.kpis.adr.title'))

@section('content')
<div class="pl-1 pr-2 py-0">
    <div class="flex flex-col sm:flex-row justify-between items-center">
        <h2 class="title-section mb-0"><?= t('modules.relatorios.kpis.adr.title') ?></h2>
    </div>
    <p class="text-sm text-slate-500 mb-3"><?= t('modules.relatorios.kpis.adr.description') ?></p>
    ...
    <div id="reportTableContainer" class="bg-white shadow-md rounded-lg overflow-x-auto" style="display: none;">
        <table class="w-full min-w-full divide-y divide-slate-200">
            <thead class="table-header-custom">
                <tr>
                    <th class="table-header"><?= t('modules.relatorios.kpis.adr.col_grupo') ?></th>
                    <th class="table-header text-right"><?= t('modules.relatorios.kpis.adr.col_receita') ?></th>
                    <th class="table-header text-center"><?= t('modules.relatorios.kpis.adr.col_dias') ?></th>
                    <th class="table-header text-right"><?= t('modules.relatorios.kpis.adr.col_adr') ?></th>
                </tr>
            </thead>
...
<script>
(function () {
    const API_URL = '/api/relatorios/kpis/adr';
    let chartInstance = null;

    const totalsConfig = [
        { key: 'receita_total', label: '<?= t("modules.relatorios.kpis.adr.receita_total") ?>', icon: 'fa-dollar-sign', format: 'currency' },
        { key: 'dias_locados', label: '<?= t("modules.relatorios.kpis.adr.dias_locados") ?>', icon: 'fa-calendar-check', format: 'number' },
        { key: 'adr', label: '<?= t("modules.relatorios.kpis.adr.adr") ?>', icon: 'fa-calculator', format: 'currency', color: 'green' },
    ];
    ...
    async function init() {
        ...
        document.getElementById('btnExportPdf')?.addEventListener('click', () => ReportUtils.exportPdf('/relatorios/kpis/adr/pdf', '<?= t("modules.relatorios.kpis.adr.title") ?>'));
    ...
    function renderChart(chartData) {
        ...
        chartInstance = new Chart(document.getElementById('reportChart').getContext('2d'), {
            type: 'line', data: { labels: chartData.labels, datasets: chartData.datasets.map((ds, i) => ({ label: ds.label, data: ds.data, borderColor: ReportUtils.COLORS[i], backgroundColor: ReportUtils.COLORS_ALPHA[i], fill: true, tension: 0.3 })) },
    ...
    function renderTable(data) {
        ...
        tbody.innerHTML = data.map(row => `<tr class="hover:bg-slate-50">
            ...
            <td class="table-cell text-right font-medium">${Currency.format(row.adr, true)}</td>
        </tr>`).join('');
    }
