...
<div class="pl-1 pr-2 py-0">
    ...
    <div class="flex items-center justify-between mb-4">
        <h2 class="title-page" id="pageTitle"><?= t('modules.contratos.new_title') ?></h2>
    ...
    <form id="formContrato" method="POST">
        ...
        <input type="hidden" id="contratoStatus" name="status" value="A">

        <!-- Cabecalho do Contrato (sempre visivel) -->
        <div class="form-section mb-4">
            <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                <!-- Filial -->
                <div class="md:col-span-3 form-input-group">
                    <label for="id_matriz_filial_retirada" class="form-label-group">
                        <?= t('modules.contratos.fields.branch') ?> <span class="text-red-500">*</span>
                    </label>
                ...
                <div class="md:col-span-2 form-input-group">
                    <label for="data_ini" class="form-label-group">
                        <?= t('modules.contratos.fields.start_date') ?> <span class="text-red-500">*</span>
                    </label>
                    <input type="datetime-local" id="data_ini" name="data_ini" class="form-input-group-field" required>
                </div>
        ...
        <div id="tabFinanceiro" class="form-tab-content">
            <!-- SECAO: CONFIGURACAO DE PAGAMENTO -->
            <div class="form-section mb-4">
                <h3 class="form-section-title"><i class="fas fa-cog mr-2"></i><?= t('modules.contratos.sections.payment_config') ?></h3>

                <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                    <div class="md:col-span-3 form-input-group">
                        <label for="id_conta" class="form-label-group"><?= t('modules.contratos.fields.bank_account') ?></label>
                    ...
                    <div class="md:col-span-2 form-input-group">
                        <label for="id_forma_pagamento" class="form-label-group"><?= t('modules.contratos.fields.payment_method') ?></label>
                    ...
                    <div class="md:col-span-2 form-input-group">
                        <label for="id_comando_parcela" class="form-label-group"><?= t('modules.contratos.fields.installment_command') ?></label>
                    ...
                    <div class="md:col-span-2 form-input-group">
                        <label for="primeiro_vencimento" class="form-label-group"><?= t('modules.contratos.fields.first_due_date') ?></label>
                        <input type="date" id="primeiro_vencimento" name="primeiro_vencimento" class="form-input-group-field">
                    </div>
                    <div class="md:col-span-3 form-input-group">
                        <label for="valor_desconto" class="form-label-group"><?= t('modules.contratos.fields.discount') ?></label>
                        <div class="relative">
                ...
                <!-- Painel informativo do comando de parcelas -->
                <div id="infoComandoParcelas" class="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg hidden">
                    <div class="flex items-start gap-2">
                        <i class="fas fa-info-circle text-blue-500 mt-0.5"></i>
                        <div class="text-sm text-blue-700">
                            <strong><?= t('modules.contratos.financial.installment_info') ?></strong> <code id="comandoParcelasTexto" class="bg-blue-100 px-1.5 py-0.5 rounded text-xs font-mono"></code>
                            <div id="comandoParcelasDescricao" class="mt-1 text-blue-600"></div>
                        </div>
                    </div>
                </div>

                <div class="mt-4 flex gap-2">
                    <button type="button" id="btnGerarParcelas" class="btn-primary py-2 px-4 rounded-md text-sm font-medium flex items-center">
                        <i class="fas fa-calculator mr-2"></i><?= t('modules.contratos.financial.generate_installments') ?>
                    </button>
                    <button type="button" id="btnLimparParcelas" class="btn-secondary py-2 px-4 rounded-md text-sm font-medium flex items-center hidden">
                        <i class="fas fa-trash mr-2"></i><?= t('modules.contratos.financial.clear_all') ?>
                    </button>
                </div>
            </div>
...
<!-- Template para Card de Veiculo (lista simplificada) -->
...
<script>
    ...
    window.i18n_contratos = <?= json_encode([
        ...
        'finalizedValuesLocked' => t('modules.contratos.financial.finalized_values_locked'),
        'finalizedVehiclePlan' => t('modules.contratos.financial.finalized_vehicle_plan'),
        'finalizedPeriod' => t('modules.contratos.financial.finalized_period'),
        'finalizedRate' => t('modules.contratos.financial.finalized_rate'),
        'finalizedSubtotal' => t('modules.contratos.financial.finalized_subtotal'),
        'finalizedSnapshotEmpty' => t('modules.contratos.financial.finalized_snapshot_empty'),
        'finalizedTotalFees' => t('modules.contratos.financial.finalized_total_fees'),
        'finalizedDepositSeparate' => t('modules.contratos.financial.finalized_deposit_separate'),
        'finalizedTotals' => t('modules.contratos.financial.finalized_totals'),
        'finalizedAdjustmentRemoved' => t('modules.contratos.financial.finalized_adjustment_removed', ['amount' => ':amount']),
        'finalizedAdjustmentRemoveTitle' => t('modules.contratos.financial.finalized_adjustment_remove_title'),
        'finalizedAdjustmentRemoveReason' => t('modules.contratos.financial.finalized_adjustment_remove_reason'),
        'finalizedAdjustmentReasonRequired' => t('modules.contratos.financial.finalized_adjustment_reason_required'),
...
(function() {
    ...
    window.addEventListener('message', function(event) {
        if (event.data && event.data.action === 'cartaoLocacaoSalvo') {
            const idCliente = event.data.id_cliente;
