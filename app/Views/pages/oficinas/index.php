...
@section('title', '<?= t("modules.oficinas.title") ?>')

@section('content')
<div class="pl-1 pr-2 py-0">
    <div class="flex flex-col sm:flex-row justify-between items-center mb-6">
        <h2 class="title-section mb-3 sm:mb-0"><?= t('modules.oficinas.title') ?></h2>
        <div class="flex items-center space-x-3 w-full sm:w-auto">
            <div class="relative flex-grow sm:flex-grow-0">
                <input type="text" placeholder="<?= t('modules.oficinas.placeholders.search') ?>" class="form-input-focus sm:w-64 pr-8" id="searchInput">
    ...
    <div class="bg-white shadow-md rounded-lg overflow-x-auto">
        <table class="w-full min-w-full divide-y divide-slate-200">
            <thead class="table-header-custom">
                <tr>
                    <th class="table-header"><?= t('modules.oficinas.table.company') ?></th>
                    <th class="table-header hidden sm:table-cell"><?= t('modules.oficinas.table.email') ?></th>
                    <th class="table-header hidden md:table-cell"><?= t('modules.oficinas.table.phone') ?></th>
                    <th class="table-header px-2 w-32 text-center"><?= t('modules.oficinas.table.actions') ?></th>
                </tr>
            </thead>
    ...
    <div class="table-pagination-controls mt-4 flex flex-wrap justify-between items-center">
        <div>
            <label for="rowsPerPage" class="text-sm text-slate-600 mr-2"><?= t('modules.oficinas.pagination.rows_per_page') ?></label>
...
(function () {
    ...
    async function carregarDados(page = 1, recordsPerPage = 10, search = '') {
        try {
            ...
            const result = await API.get('/api/oficinas', {
                page: page,
    ...
    function renderDados(items) {
        ...
        let tableRows = '';
        items.forEach(item => {
            const empresa = item.empresa || i18n.noName;
            const email = item.email || '-';
            const telefone = item.telefone || '-';
            const empresaEscapada = escapeHtml(empresa);

            tableRows += `
                <tr class="border-b border-slate-200 hover:bg-slate-50">
                    <td class="table-cell">
                        <div class="font-medium">${empresaEscapada}</div>
                    </td>
                    <td class="table-cell hidden sm:table-cell text-slate-600">${escapeHtml(email)}</td>
                    <td class="table-cell hidden md:table-cell text-slate-600">${escapeHtml(telefone)}</td>
                    <td class="table-cell px-2 w-32 text-right">
                        ...
                        <button title="${i18n.actionDelete}" class="btn-icon text-red-600 hover:text-red-800 btn-delete" data-id="${item.id}" data-name="${empresaEscapada}"><i class="fas fa-trash"></i></button>
                    </td>
                </tr>
            `;
        });
        ...
        tbody.querySelectorAll('.btn-edit').forEach(button => {
            button.addEventListener('click', function () {
                ...
                navegarPara('/pages/oficinas/adicionar?id=' + id);
            });
        });
    ...
    document.getElementById('btnNovo')?.addEventListener('click', function () {
        navegarPara('/pages/oficinas/adicionar');
    });
    ...
    async function excluirRegistro(id) {
        try {
            const result = await API.post(`/oficinas/${id}/excluir`);
