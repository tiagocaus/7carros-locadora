    ...
    <form id="formLocacao" method="POST">
        ...
        <div id="tabFinanceiro" class="form-tab-content">
            ...
            <div id="secaoBloqueio" class="form-section mb-4">
                ...
                <div id="bloqueioEstadoNovo" class="<?= isset($locacao) ? 'hidden' : '' ?> rounded-md border border-slate-200 bg-slate-50 px-4 py-6 text-center text-sm text-slate-600">
                    <i class="fas fa-info-circle mr-2 text-slate-400"></i><?= t('modules.locacoes.form.save_before_hold') ?>
                </div>

                <div id="bloqueioFormFields" class="<?= isset($locacao) ? '' : 'hidden' ?>">
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
...
<script>
    window.SINISTROS_CONFIG = {
        ...
        canDelete: <?= \App\Core\Auth::can('locacoes.editar') ? 'true' : 'false' ?>,
        ...
        document.getElementById('btnConfirmarGerarParcelas')?.addEventListener('click', async () => {
            ...
            try {
                const result = await API.post(`/api/locacoes/${locacaoData.id}/gerar-parcelas`, {
                    ...
                    odometro_ini: Km.parse(document.getElementById('odometro_ini')?.value || '0'),
                    odometro_fim: Km.parse(document.getElementById('odometro_fim')?.value || '0'),
