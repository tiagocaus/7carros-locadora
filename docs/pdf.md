...
Nos cabecalhos, email e telefone da matriz/filial devem vir dos aliases
normalizados entregues por `MatrizFilial` (`email` e `telefone`). Esses valores
sao resolvidos em `contatos_emails` e `contatos_telefones`; nao existem mais
colunas diretas de contato em `matrizes_filiais`.
